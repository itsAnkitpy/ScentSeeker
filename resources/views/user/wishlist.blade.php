                        ...
                        <div class="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <button @click="showCreateModal = true"
                                    class="w-8 h-8 bg-teal-500 text-white rounded-full flex items-center justify-center hover:bg-teal-600 transition-colors">
                            ...
                            <div class="space-y-2">
                                <template x-for="list in wishlists" :key="list.id">
                                    <button @click="selectWishlist(list)"
                                        :class="{'bg-teal-100 border-teal-400': selectedWishlist?.id === list.id, 'bg-gray-50 border-gray-200': selectedWishlist?.id !== list.id}"
                                        class="w-full text-left px-4 py-3 rounded-xl border-2 transition-all hover:border-teal-300">
                            ...
                            <div class="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <template x-for="item in (selectedWishlist.items || [])" :key="item.id">
                                        <div class="bg-gradient-to-r from-teal-50 to-cyan-50 rounded-xl p-4 flex gap-4">
                                            ...
                                            <div class="flex-1">
                                                <p class="text-xs text-teal-600 font-bold" x-text="item.perfume?.brand"></p>
                                                ...
                                                <div class="mt-2 flex gap-2">
                                                    <a :href="`/perfumes/${item.perfume_id}`"
                                                        class="text-xs text-teal-600 hover:underline">View</a>
            ...
            <div x-show="showCreateModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div class="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
                    ...
                    <input type="text" x-model="newWishlistName" placeholder="Wishlist name..."
                        class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-teal-400 mb-4">
                    <div class="flex gap-4">
                        <button @click="createWishlist()"
                            class="flex-1 bg-gradient-to-r from-teal-500 to-teal-600 text-white py-3 rounded-xl font-semibold">
                            Create
                        </button>
